...
class RNNConfig(object):
    ...
    rnn='gru'
    train_filename = './data4/train.txt'  # train data
    test_filename = './data4/test.txt'  # test data
    val_filename = './data4/test.txt'  # validation data
    vocab_filename = './data4/cnews.vocab.txt'  # vocabulary
    vector_word_filename = './data4/cnews.vector_word.txt'  # vector_word trained by word2vec
    vector_word_npz = './data4/cnews.vector_word.npz'  # save vector_word to numpy file
    ...
    def rnn(self):
        ...
        with tf.device('/cpu:0'):#5000行64列代表一个字
            embedding = tf.get_variable('embedding', [self.config.vocab_size, self.config.embedding_dim]
                                        ,initializer=tf.constant_initializer(self.config.pre_trianing))#(5000,64)5000个字
